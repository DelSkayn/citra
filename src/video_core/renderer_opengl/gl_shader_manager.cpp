...
std::shared_ptr<PicaShader> GLShaderManager::get(const Pica::State & state,const Pica::Shader::ShaderSetup & vs, bool & cached){
    ...
    if(shader == shaders.end()){
        ...
        auto source = Pica::Shader::Decompiler::Decompiler::decompile(vs.program_code,vs.swizzle_data,config.state.entry_point);
        if(source){
            n_shader->shader.Create( source->c_str()
